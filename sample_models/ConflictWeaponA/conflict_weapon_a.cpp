...
// 与 B 共用共享内存。仅当 B 也在存活时 Init/Step 失败。
// 同型号多线程（多个 A）应能通过（不把 activeCount>1 当冲突）。
static const wchar_t* kSharedMapping = L"Local\\ModelPrecheck_ConflictWeaponShared";
static const unsigned int kOwnerTag = 0x00000041u;
...
WEAPON_EXPORT int Model_Init(const WeaponModelParams* params) {
    ...
    InterlockedIncrement(&g_shared->countA);
    g_shared->lastOwnerTag = kOwnerTag;

    bool sawB = false;
    const DWORD t0 = GetTickCount();
    while (GetTickCount() - t0 < kOverlapWaitMs) {
        if (g_shared->countB > 0) sawB = true;
        Sleep(10);
    }
    if (sawB) {
        InterlockedDecrement(&g_shared->countA);
        UnmapShared();
        return -2;
    }
...
WEAPON_EXPORT int Model_Step(WeaponModelOutput* output) {
    ...
    if (g_shared->countB > 0) return -99;
    ...
    // A：每隔一段直角拐弯，形成折线而非单向直线
    if ((g_shared->stepCount % 22) == 1) {
        g_shared->yaw += 90.0;
    }
    g_shared->yaw += 8.0 * std::sin(g_shared->simTime * 1.1);

    double radPitch = g_shared->pitch * pi / 180.0;
    double radYaw = g_shared->yaw * pi / 180.0;
    double vx = g_shared->speed * std::cos(radPitch) * std::cos(radYaw);
    double vy = g_shared->speed * std::cos(radPitch) * std::sin(radYaw);
    double vz = g_shared->speed * std::sin(radPitch) - 0.5;
...
WEAPON_EXPORT const char* Model_GetInfo() {
    return "ConflictWeaponA v1.4 — right-angle turns; FAIL if B active";
}
