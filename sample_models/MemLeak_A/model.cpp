...
struct State {
    ...
    bool inited;
    char* orphan; // allocated in Init, never freed
};
...
int Model_Init(void* handle, const WeaponModelParams* params) {
    ...
    // INTENTIONAL LEAK: ~64KB kept forever (Destroy does not free it).
    // Keep modest so default 10k UserMain stress can finish / early-abort
    // instead of allocating tens of GB (old 256KB Init + 64KB/step).
    s->orphan = static_cast<char*>(std::malloc(64 * 1024));
    if (s->orphan) {
        std::memset(s->orphan, 0xA5, 64 * 1024);
    }
    return 0;
}
...
int Model_Step(void* handle, WeaponModelOutput* output) {
    ...
    // INTENTIONAL LEAK: ~1KB every step (touch page so Working Set grows)
    char* leak = static_cast<char*>(std::malloc(1024));
    if (leak) {
        std::memset(leak, 0x5A, 1024);
    ...
    output->vy = ve;
    output->vz = vd;
    output->pitch = s->pitch_deg;
    output->roll = s->roll_deg;
    output->yaw = s->heading_deg;
    output->status = 0;
    return 0;
}
...
void Model_Destroy(void* handle) {
    if (!handle) return;
    State* s = static_cast<State*>(handle);
    // INTENTIONAL: do NOT free s->orphan
    s->orphan = nullptr;
    delete s;
}

const char* Model_GetInfo(void) {
    return "MemLeak_A | INTENTIONAL: Init 64KB + Step 1KB/step never freed";
}
