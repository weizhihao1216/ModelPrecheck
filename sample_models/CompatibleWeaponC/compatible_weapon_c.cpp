...
WEAPON_EXPORT int Model_Init(const WeaponModelParams* params) {
    ...
    tls.baseYaw = params->init_heading;
...
WEAPON_EXPORT int Model_Step(WeaponModelOutput* output) {
    ...
    // C：正弦拐弯（S 形），不沿直线
    tls.yaw = tls.baseYaw + 55.0 * std::sin(tls.simTime * 1.4)
                           + 18.0 * std::sin(tls.simTime * 0.35);
    tls.pitch = tls.params.init_pitch + 4.0 * std::sin(tls.simTime * 0.8);

    const double radPitch = tls.pitch * pi / 180.0;
    const double radYaw = tls.yaw * pi / 180.0;
    const double groundScale = 60.0;
    ...
    const double vz = tls.speed * std::sin(radPitch) - 0.45;
...
WEAPON_EXPORT const char* Model_GetInfo() {
    return "CompatibleWeaponC v1.2 — S-curve lat/lon (out_lat/out_lon)";
}
